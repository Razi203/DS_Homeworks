    ...
    for line in fin.readlines():
        if re.search("#([\\s]*)include([\\s]*)\\<([^\\>]*)\\>", line) != None:
            ...
            if inc.lower() in deniedh:
                err = err + 1
                print(
                    "Error: found denied header in " + path + ": include <" + inc + ">"
                )
                continue
...
def verify_files(dir, content):
    ...
    if codefiles == 0:
        print(
            "Error: no code files! Shouldn't have a directory in the top level of zip"
        )
        return 1
...
def main_verify():
    ...
    if nerr == 0:
        if len(sys.argv) > 2:
            if "--hash" in sys.argv:
                print(
                    "The submission file has hash: "
                    + str(hash(open(path_to_zip, "r").read()))
                )
